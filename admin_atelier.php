...
<body class="sub_page">
  ...
  <section class="contact_section layout_padding">
    ...
    <div class="container">
      ...
      <?php
          //affichage tableau pad - atelier - ressource
          $requete = "SELECT pad_intitule, at_id, at_intitule, GROUP_CONCAT( DISTINCT rsc_titre SEPARATOR ', ') as rsc_titre, GROUP_CONCAT( DISTINCT cpt_pseudo SEPARATOR ', ') as animateurs FROM t_ressource_rsc RIGHT JOIN t_atelier_at USING(at_id) RIGHT JOIN t_pad_pad USING(pad_id) RIGHT JOIN t_animation_an USING(pad_id) JOIN t_compte_cpt USING(cpt_pseudo) JOIN t_profil_pfl USING(cpt_pseudo) GROUP BY at_id ORDER BY at_date DESC;";
          ...
          { // La requête a echoué
              ...
              while ($res = $result->fetch_assoc()) {
                  echo ('<tr>');
                  echo ('<td>' . $res['pad_intitule'] . '</td><td>'.$res['animateurs'].'</td><td>'.stripslashes($res['at_intitule']).'</td><td>'.$res['rsc_titre'].'</td>');
    ...
    <section class="contact_section layout_padding">
      ...
      <form action="admin_atelier_action.php" method="post" class="form">
        <select id="pad_id" name="pad_id" class="form-select">
          <option selected>Choisir un pad</option>
          <?php
            //liste deroulante selection pad
        ...
        <input id="intitule" name="intitule" type="text" placeholder="INTITULE">
        <input id="description" name="description" type="text" placeholder="DESCRIPTION">
        <input id="etat" name="etat" type="text" placeholder="ETAT">
